...
from conf_info_ui import ConfInfoUI
from Next_trial_info_ui import NextInfoUI
from End_info_ui import EndInfoUI
from Start_info_ui import StartInfoUI
from expstart_info_ui import ExpStart
...
class LTAPCarlaClient():
    ...
    def __init__(self):
        try:
            ...
            self.n_routes_per_session = 6

            self.initialize_log()
            self.initialize_log_conf()
            ...
            self.tta_conditions = [5.5, 6.5]
            self.bot_distance_values = [70, 90]
            self.n_intersection = 20
    ...
    def generate_tta_values(self):
        tta_values = []
        distance_values = []
        i = int((self.n_intersection * 0.8) / 2)

        for tta in self.tta_conditions:
            tta_values = np.append(tta_values, np.ones(i) * tta)

        for _ in range(2):
            for distance in self.bot_distance_values:
                distance_values = np.append(distance_values, np.ones(int(i / 2)) * distance)
        ...
        distance_values = distance_values[ii]

        print(len(tta_values))

        for ii in range(len(tta_values)):
            print(tta_values[ii], distance_values[ii])

        np.savetxt('tta_values.txt', tta_values, fmt='%.2f')
        np.savetxt('distance_values.txt', distance_values, fmt='%.2f')

        return tta_values, distance_values

    def initialize_log(self):
        log_directory = 'data'
        self.log_file_path = os.path.join(log_directory, str(self.exp_info['subj_id']) + '_' +
                                          self.exp_info['start_time'] + 'log.txt')
        with open(self.log_file_path, 'w') as fp:
            writer = csv.writer(fp, delimiter='\t')
            writer.writerow(['subj_id', 'route', 'int_no',
                             'int_x', 'int_y', 'turn_dir', 't',
                             'ego_dis_int', 'bot_dis_int', 'tta_con', 'd_con', 'v_con', 'truck_ang', 'bot_ang',
                             'ego_x', 'ego_y', 'ego_vx', 'ego_vy', 'ego_ax', 'ego_ay', 'ego_yaw',
                             'bot_x', 'bot_y', 'bot_vx', 'bot_vy', 'bot_ax', 'bot_ay', 'bot_yaw',
                             'throttle', 'brake', 'steer', 'Go', 'let_pass',
    ...
    def initialize_log_conf(self):
        log_directory = 'conf data'
        self.log_conf_file_path = os.path.join(log_directory, str(self.exp_info['subj_id']) + '_' +
                                               self.exp_info['start_time'] + 'conf.txt')
        with open(self.log_conf_file_path, 'w') as fp:
            writer = csv.writer(fp, delimiter='\t')
            writer.writerow(['subj_id', 'route', 'int_no',
                             'conf', 'tta_con', 'd_con', 'go', 'wait', 't_paused', 't_conf'])

    def write_conf_log(self, log):
        with open(self.log_conf_file_path, 'a') as fp:
            writer = csv.writer(fp, delimiter='\t')
            writer.writerows(log)
    ...
    def get_conf_info(self):
        root = tkr.Tk()
        app = ConfInfoUI(master=root)
        app.master.title("Self-reflection")
        app.master.minsize(700, 400)

        # Position of the window on the screen
        screen_w = root.winfo_screenwidth()
        screen_h = root.winfo_screenheight()
        center_x = int(screen_w / 2 - 350)
        center_y = int(screen_h / 2 - 300)
        root.geometry(f'{700}x{400}+{center_x}+{center_y}')

        app.mainloop()
        conf_info = app.conf_info
        root.destroy()
    ...
    def get_next_info(self):
        root = tkr.Tk()
        app = NextInfoUI(master=root)
        app.master.title("Next route")
        app.master.minsize(400, 250)

        # Position of the window on the screen
        screen_w = root.winfo_screenwidth()
        screen_h = root.winfo_screenheight()
        center_x = int(screen_w / 2 - 200)
        center_y = int(screen_h / 2 - 300)
        root.geometry(f'{400}x{250}+{center_x}+{center_y}')

        app.mainloop()
        next_info = app.next_info
        root.destroy()
    ...
    def get_end_info(self):
        root = tkr.Tk()
        app = EndInfoUI(master=root)
        app.master.title("End")
        app.master.minsize(700, 250)
        ...
        center_x = int(screen_w / 2 - 350)
        center_y = int(screen_h / 2 - 300)
        root.geometry(f'{700}x{250}+{center_x}+{center_y}')
    ...
    def update_ego_control(self):
        # Do not know if pygame.event.pump() is needed, however it is needed to print updated values
        pygame.event.pump()
        reverse = False

        # K1=0.5 for real steer
        K1 = 1
        ...
        # Driving reverse when pressing the R3 button
        ...
        self.Go_last = self.Go

        self.let_pass = self.joystick.get_button(4)  # The right paddle
        self.Go = self.joystick.get_button(2)  # The O-button

        if (self.let_pass == 1) & (self.let_pass_last == 0):
            sound = pygame.mixer.Sound('sounds/LetPass0.wav')
            sound.set_volume(0.5)
            sound.play()

        if (self.Go == 1) & (self.Go_last == 0):
            sound = pygame.mixer.Sound('sounds/LetPass0.wav')
            sound.set_volume(0.5)
            sound.play()
    ...
    def spawn_ego_car(self):
        ...
        # ego_bp = random.choice(self.world.get_blueprint_library().filter('vehicle.hapticslab.epicaudi_realsteer'))
    ...
    def update_bot_control(self, v0):
        # self.bot_actor.set_target_velocity(carla.Vector3D(self.bot_velocity[0], -self.bot_velocity[1], 0))
        bot_velocity_dynamic = v0 * self.bot_direction
    ...
    def get_anlge_of_sight(self):
        ...
        if not self.bot_actor is None:
            ...
            bot_corner_bodyframe = np.array(
                [self.bot_actor.bounding_box.location.x, self.bot_actor.bounding_box.location.y]) + \
                                   np.array(
                                       [self.bot_actor.bounding_box.extent.x, self.bot_actor.bounding_box.extent.y])
        ...
        if not self.truck_actor is None:
            ...
            truck_corner_bodyframe = np.array(
                [self.truck_actor.bounding_box.location.x, self.truck_actor.bounding_box.location.y]) + \
                                     np.array([-self.truck_actor.bounding_box.extent.x,
                                               -self.truck_actor.bounding_box.extent.y])
            # Center of Bounding Box
        ...
        if self.joystick.get_button(23):
            self.world.debug.draw_arrow(B_carla_location, self.origin_loc, persistent_lines=False, life_time=0.01,
                                        color=carla.Color(0, 255, 0, 0))
            self.world.debug.draw_arrow(self.origin_loc, self.active_intersection_loc, persistent_lines=False,
                                        life_time=0.01)
        ...
        if (not self.bot_actor is None) & (self.debug_flag is True):
            ...
            self.world.debug.draw_arrow(B_carla_location, C_carla_location, persistent_lines=False, life_time=0.01,
                                        color=bot_arrow_color)
            # print('C (bot) :', C)
    ...
    def update_log(self, log, values_to_log):
        decision_feedback = list(
            ['%i' % value for value in [self.Go, self.let_pass]])
        log.append((values_to_log +
                    self.get_actor_state(self.ego_actor) +
                    self.get_actor_state(self.bot_actor) +
                    list(
                        ['%.4f' % value for value in [self.control.throttle, self.control.brake, self.control.steer]]) +
                    decision_feedback +
                    self.get_actor_state(self.truck_actor)))

    def update_conf_log(self, log, values_to_log):
        log.append(values_to_log)

    def run(self):
        self.get_start_info()
        session = self.exp_info['session']
        ...
        try:
            ...
            self.n_routes_per_session = self.n_routes_per_session - first_route + 1
            previous_value = self.exp_info['int_nbr']
            print('nbr routes', self.n_routes_per_session)

            while pilot:
                print('PILOT')
                route = np.loadtxt(os.path.join('routes', 'route_pilot.txt'))

                tta_values, distance_test = self.generate_tta_values
                self.origin = np.array([0.0, 0.0])
                self.active_intersection = np.array([1.0, 0.0])
                ...
                for j, current_turn in enumerate(route):
                    ...
                    # the same TTA will be used for next trials until there's another left turn
                    print('current_turn', current_turn)
                    if (current_turn == 1):
                        tta_condition = tta_values[-1]
                        tta_values = tta_values[:-1]

                        d_condition = distance_test[-1]
                        distance_test = distance_test[:-1]

                    bot_speed = d_condition / tta_condition
                    ...
                    while not is_turn_completed:
                        ...
                        speed = np.sqrt(self.ego_actor.get_velocity().x ** 2 + self.ego_actor.get_velocity().y ** 2)
                        ego_distance_to_intersection = self.ego_actor.get_location().distance(
                            self.active_intersection_loc)
                        ...
                        if not self.bot_actor is None:
                            ...
                            bot_distance_from_spawn_location = self.bot_actor.get_location().distance(
                                self.bot_spawn_location_target)
                            if self.debug_flag is True:
                                print('Angle of Sight :', angle_of_sight, ' Bot Location:',
                                      self.bot_actor.get_location(), ' Delta =', bot_distance_from_spawn_location,
                                      self.bot_move)
                        ...
                        # if at the right turn, don't wait for slowdown when spawning a bot
                            ...
                            # truck_speed = 30.0
                        elif ((angle_of_sight > 0.0) & (self.bot_move is False) & (not self.bot_actor is None) & (
                                bot_distance_from_spawn_location < 0.01)):
                            self.bot_move = True
                            self.bot_move_start_time = time.time()
                        ...
                        # When the driver leaves the intersection we designate the next intersection as active and destroy the bot
                            ...
                            if current_turn == 1:
                                paused = True
                                rating = True
                                self.noise_sound.set_volume(0.0)

                                while paused:
                                    self.ego_actor.enable_constant_velocity(carla.Vector3D(0, 0, 0))
                                    pygame.event.pump()
                                    ...
                                    # Pressing L3 will let the driving task resume
                                    if self.joystick.get_button(11):
                                        print('Play')
                                        paused = False
                                        self.ego_actor.disable_constant_velocity()
                ...
                time.sleep(0.1)
                self.next_info = self.get_next_info()
                next_tr = self.next_info['next']
                if next_tr == 1:
                    pilot = False
                    print('END PILOT')
                time.sleep(0.1)

            for i in range(first_route, first_route + self.n_routes_per_session):
                if i == first_route:
                    self.start_exp()
                else:
                    previous_value = 0
                ...
                if previous_value == 0:
                    ...
                    tta_values = np.loadtxt(os.path.join('tta_values.txt'))
                    distance_test = np.loadtxt(os.path.join('distance_values.txt'))
                    tta_values = tta_values[1:-previous_value]
                    distance_test = distance_test[1:-previous_value]
                    for ii in range(len(tta_values)):
                        print(tta_values[ii], distance_test[ii])
                ...
                for j, current_turn in enumerate(route):
                    ...
                    if previous_value == 0 or (j > (previous_value - 1)) & (current_turn > 0) or inloop is True:
                        ...
                        while not is_turn_completed:
                            ...
                            if self.bot_actor is None:
                                bot_distance_to_intersection = 99999

                            self.get_actor_state(self.bot_actor)

                            '''
                                'subj_id', 'route', 'int_no',
                                'int_x', 'int_y', 'turn_dir', 't',
                                'ego_dis_to_int', 'tta_con', 'd_con', 'v_con' , 'truck_ang', 'bot_ang'
                                '''

                            values_to_log = list(['%i' % value for value in
                                                  [self.exp_info['subj_id'], route_number, j + 1,
                                                   intersection_coordinates[0], intersection_coordinates[1],
                                                   current_turn]]) + list(['%.4f' % value for value in
                                                                           [t, ego_distance_to_intersection, bot_distance_to_intersection, tta_condition,
                                                                            d_condition, bot_speed, self.truck_angle,
                                                                            self.bot_angle]])
                            ...
                            if wait_decision == 1 & self.Go == 1:
                                go_decision = 2

                            if go_decision == 1 & self.let_pass == 1:
                                wait_decision = 2
                            ...
                            if not self.bot_actor is None:
                                ...
                                bot_distance_from_spawn_location = self.bot_actor.get_location().distance(
                                    self.bot_spawn_location_target)
                                bot_distance_to_intersection = self.bot_actor.get_location().distance(
                                    self.active_intersection_loc)
                                if self.debug_flag is True:
                                    print('Angle of Sight :', angle_of_sight, ' Bot Location:',
                                          self.bot_actor.get_location(), ' Delta =', bot_distance_from_spawn_location,
                            ...
                            # When the driver leaves the intersection we designate the next intersection as active and destroy the bot
                                ...
                                if current_turn == 1:
                                    paused = True
                                    t_paused = time.time() - trial_start_time
                                    rating = True
                                    self.noise_sound.set_volume(0.0)
                                    ...
                                    while paused:
                                        ...
                                        if rating is True:
                                            self.conf_info = self.get_conf_info()
                                            t_conf = time.time() - trial_start_time
                                            rating = False

                                        print('paused')

                                        # Pressing L3 will let the driving task resume
                                        if ((self.joystick.get_button(11)) & (rating is False)):
                                            print('Play')
                                            paused = False
                                            self.ego_actor.disable_constant_velocity()

                                            conf = self.conf_info['conf']

                                            values_to_conf_log = list(['%i' % value for value in
                                                                       [self.exp_info['subj_id'], route_number,
                                                                        j + 1]]) + list(['%.2f' % value for value in
                                                 [conf, tta_condition, d_condition, go_decision, wait_decision]]) + list(['%.4f' % value for value in
                                                                                         [t_paused, t_conf]])

                                            self.update_conf_log(conf_log, values_to_conf_log)


                            time.sleep(0.01)

                        self.write_log(trial_log)
                        self.write_conf_log(conf_log)
                ...
                if i < (first_route + self.n_routes_per_session - 1):
                    wait = True
                else:
                    wait = False
                    time.sleep(0.1)
