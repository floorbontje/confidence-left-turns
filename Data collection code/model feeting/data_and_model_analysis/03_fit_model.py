...
import loss_functions_go_wait
import pandas as pd
import helper
import os


def fit_model_by_condition(model_no=0, subj_idx='all', n=1, n_training_conditions=9, test_conditions="all"):
    '''
    ...
    exp_data = pd.read_csv("../data/data_output_thr.csv", usecols=["subj_id", "RT", "is_go_decision",
    ...
    if subj_idx == "all":
        subj_id = "all"
        subj_data = exp_data
        # loss = pyddm.models.LossRobustLikelihood
        # loss = pyddm.models.LossRobustBIC
        loss = loss_functions.LossWLSVincent
        # loss = loss_functions_go_wait.LossWLSVincent

    else:
        subj_id = subjects[subj_idx]
        subj_data = exp_data[(exp_data.subj_id == subj_id)]
        loss = loss_functions.LossWLS
        # loss = loss_functions_go_wait.LossWLS
        # loss = pyddm.models.LossRobustLikelihood
        # loss = pyddm.models.LossRobustBIC
...
# fit_model_by_condition(model_no=1, n_training_conditions=9, subj_idx=16)
fit_model_by_condition(model_no=0, n_training_conditions=9, subj_idx='all')
